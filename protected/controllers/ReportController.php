  ...
  class ReportController extends Controller{
    ...
    public function actionDailyRevenue(){
      ...
      if(isset($_POST['Report'])){
        ...
        if($model->validate()){
          $sql = "SELECT ves.name vessel,ves.id vesid, voy.name voyage,voy.id voyeid,b.departure_date,s.name classname,s.id sclassid,count(*) count,rt.name routname,SUM(r.price) amount  FROM booking b,ticket t,voyage voy,vessel ves, passage_fare_rates r,seating_class s,route rt WHERE b.ticket = t.id AND t.voyage = voy.id AND voy.vessel = ves.id AND t.rate =r.id  AND r.class = s.id  AND voy.route=rt.id AND ves.id={$model->vessel} AND b.departure_date = '{$model->departure_date}'  GROUP BY ves.id,voy.id,s.id ";
